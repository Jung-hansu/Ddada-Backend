...
public class CourtController {
    ...
    @GetMapping
    public CommonResponse<Page<CourtSimpleResponse>> getCourtsByKeyword(@RequestParam(required = false) String keyword, @RequestParam int page, @RequestParam int size){
        log.info("시설 검색 결과 조회 >>>> 검색어: {}, 페이지 번호: {}, 페이지 크기: {}", keyword, page, size);

        Page<CourtSimpleResponse> response = courtService.getCourtsByKeyword(keyword, page, size);
